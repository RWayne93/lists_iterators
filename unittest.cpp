...
template <> std::string CppUnitLite::getStringRepr(std::string t) {
  return std::string("\"") + t + '"';
}
template <> std::string CppUnitLite::getStringRepr(const char t[]) {
  return CppUnitLite::getStringRepr(std::string(t));
}
template <> std::string CppUnitLite::getStringRepr(char t) {
  return std::string("'") + t + "'";
}
template <> std::string CppUnitLite::getStringRepr(bool b) {
  return (b) ? "true" : "false";
}

UnitTest::UnitTestFailure::UnitTestFailure(const char *conditionStr,
                                           const char *fileName,
                                           int lineNumber) {
  if (!UnitTest::expectToFail) {
    std::ostringstream out;
    out << "Failed assertion " << conditionStr << " in " << currentTest
...
UnitTest::UnitTestFailure::UnitTestFailure(const std::string &conditionStr,
                                           const char *fileName,
                                           int lineNumber) {
  if (!UnitTest::expectToFail) {
    std::ostringstream out;
    out << fileName << ":" << lineNumber << ": \t" << conditionStr << "\n";
    explanation = out.str();
  } else {
    explanation = "(expected to fail)";
  }
...
AssertionResult::AssertionResult(bool theResult, std::string pexplain,
                                 std::string fexplain)
    : result(theResult), passExplanation(pexplain), failExplanation(fexplain) {}

#ifdef __MINGW32__

bool UnitTest::debuggerIsRunning() { return IsDebuggerPresent(); }
#elif __CYGWIN__
...
void UnitTest::checkTest(AssertionResult assertionResult,
                         std::string conditionStr, const char *fileName,
                         int lineNumber) {
...
//{
//	checkTest(AssertionResult(condition), conditionStr.c_str(), fileName,
//lineNumber);
// }

// Print a simple summary report
void UnitTest::report() { UnitTest::msgSummary(); }

// Register a new UnitTest
int UnitTest::registerUT(std::string functName, int timeLimit,
                         TestFunction funct) {
  if (tests == nullptr) {
    tests = new std::map<std::string, UnitTest::BoundedTest>();
  }
  if (tests->count(functName) > 0) {
    std::cerr << "**Error: duplicate unit test named " << functName
              << std::endl;
  }
...
int UnitTest::runTestGuarded(unsigned testNumber, std::string testName,
                             TestFunction u, std::string &testExplanation) {
  ...
  try {
    ...
    if (setjmp(unitTestSignalEnv)) {
      ...
      if (!expectToFail) {
        testExplanation =
            UnitTest::msgFailed(testNumber, testName, out.str(), 0);
        return -1;
      } else {
    ...
    if (!expectToFail) {
      UnitTest::msgError(testNumber, testName,
                         "Unexpected error in " + currentTest + ": " + e.what(),
                         0);
      testExplanation = "";
      return -1;
    } else {
    ...
    if (!expectToFail) {
      UnitTest::msgError(testNumber, testName,
                         "Unexpected error in " + currentTest, 0);
      testExplanation = "";
      return -1;
    } else {
      // OK (exception but was expected to fail)"
 ...
 */
void UnitTest::expectedToFail() { expectToFail = true; }

// Run a single unit test function with no timer.
void UnitTest::runTestUntimed(unsigned testNumber, std::string testName,
                              TestFunction u) {
...
void UnitTest::runTest(unsigned testNumber, std::string testName,
                       TestFunction u, long timeLimit) {
  if (timeLimit > 0L && !debuggerIsRunning()) {
    ...
    std::thread t([&m, &testNumber, &testName, &u, &testResult,
                   &testExplanation]() {
      {
    ...
    if (testResult < -1) {
      ...
      failedTests.push_back(testName);
      std::ostringstream out;
      out << "# Test " << testNumber << " - " << currentTest
          << " still running after " << timeLimit
...
// Run a single unit test function.
void UnitTest::runTest(unsigned testNumber, std::string testName,
                       TestFunction u, long int timeLimit) {
  runTestUntimed(testNumber, testName, u);
}
...
void UnitTest::runTests(int nTests, char **testNames, char *program) {
  ...
  for (int i = 0; i < nTests; ++i) {
    ...
    if (!found) {
      badTestSpecifications +=
          "# Warning: No matching test found for input specification " +
          testID + "\n";
    }
  }
 ...
 * Clear the call log.
 */
void UnitTest::clearCallLog() { callLog.clear(); }

/**
 * Position of oldest logged call.
 */
UnitTest::iterator UnitTest::begin() { return callLog.begin(); }

/**
 * Position just after the most recently logged call.
 */
UnitTest::iterator UnitTest::end() { return callLog.end(); }
...
void UnitTest::msgPassed(unsigned testNumber, std::string testName,
                         unsigned timeMS) {
  using namespace std;
  cout << flush;
...
void UnitTest::msgXPassed(unsigned testNumber, std::string testName,
                          unsigned timeMS) {
  UnitTest::msg(UnitTest::msgFailed(
      testNumber, testName,
...
std::string UnitTest::extractLocation(const std::string &msg) {
  ...
  char c = msg[start];
  while (start <= pos && (isalnum(c) || c == '.' || c == '_' || c == '-' ||
                          c == '/' || c == '\\')) {
    --start;
    if (start <= pos)
      c = msg[start];
...
void UnitTest::msgXFailed(unsigned testNumber, std::string testName,
                          std::string diagnostics, unsigned timeMS) {
  std::string diagnosticMsg =
      msgComment(std::string("Test ") + std::to_string(testNumber) +
                 " failed but was expected to fail.");
  if (diagnosticMessagesBeforeResults)
...
void UnitTest::msgError(unsigned testNumber, std::string testName,
                        std::string diagnostics, unsigned timeMS) {
...
void UnitTest::msgSummary() {
  using namespace std;
  cout << "# UnitTest: passed " << numSuccesses << " out of " << getNumTests()
       << " tests, for a success rate of " << std::showpoint << std::fixed
       << std::setprecision(1) << (100.0 * numSuccesses) / (float)getNumTests()
       << "%" << endl;
}
...
AssertionResult StringEndsWithMatcher::eval(const std::string &e) const {
  ...
  bool result =
      (right.size() <= e.size()) &&
      equal(right.begin(), right.end(), e.begin() + e.size() - right.size());

  return AssertionResult(result, eStr + " ends with " + rightStr,
                         eStr + " does not end with " + rightStr);
}

StringEndsWithMatcher endsWith(const char *t) {
  return StringEndsWithMatcher(std::string(t));
}

StringEndsWithMatcher endsWith(const std::string &t) {
...
AssertionResult StringBeginsWithMatcher::eval(const std::string &e) const {
  ...
  return AssertionResult(result, eStr + " begins with " + rightStr,
...
CppUnitLite::NullMatcher isNull() { return CppUnitLite::NullMatcher(); }
